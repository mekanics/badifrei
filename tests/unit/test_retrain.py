...
class TestRetrainJob:
    async def test_retrain_job_runs_with_sufficient_data(self):
        ...
        mock_report.stratified = {}
        mock_report.model_rmse = 6.0
        mock_report.baseline_rmse = 9.0
        mock_report.worst_pool = "SSD-1"
        mock_report.best_pool = "SSD-2"
        mock_report.n_test = 100
    ...
    async def test_retrain_job_skips_on_insufficient_data(self):
        ...
        with (
            patch(
                "ml.retrain.load_data",
                AsyncMock(side_effect=InsufficientDataError("Not enough")),
            ),
