...
class TestIsStale:
    ...
    def test_far_beyond_ttl_is_stale(self):
        from api.weekly_insights import is_stale
...
class TestCacheHit:
    ...
    async def test_cache_hit_returns_cached_value(self, mock_predictor_spy):
        ...
        computed_at = datetime.now(timezone.utc) - timedelta(
            seconds=60
        )  # fresh (TTL=3600)
        ...
        with patch("api.routers.pages.predictor", mock_predictor_spy):
            with patch("api.routers.pages.asyncio.create_task") as mock_create_task:
...
class TestCacheMiss:
    async def test_cache_miss_triggers_background_recompute(self):
        ...
        with patch("api.routers.pages.predictor", mock_pred):
            with patch(
                "api.routers.pages.asyncio.create_task",
                ...
                def _capture(
                    self_, name_or_request, context_or_name=None, *args, **kwargs
                ):
        ...
        assert (
            context_captured.get("weekly_insights") is None
...
class TestBackgroundRefresh:
    async def test_background_refresh_updates_cache(self):
        ...
        with patch("api.weekly_insights.predictor", mock_pred):
            await refresh_weekly_insights(pool_uid, None, app.state)

        # Cache must now contain an entry for this pool
        assert (
            pool_uid in app.state.weekly_insights_cache
...
class TestTTLConfig:
    def test_ttl_configurable_via_env_var(self, monkeypatch):
        ...
        import api.weekly_insights as weekly_insights

        api_config.get_settings.cache_clear()
        importlib.reload(weekly_insights)

        assert (
            weekly_insights.WEEKLY_INSIGHTS_CACHE_TTL_SECONDS == 120
        ), f"Expected TTL=120, got {weekly_insights.WEEKLY_INSIGHTS_CACHE_TTL_SECONDS}"
    ...
    def test_default_ttl_is_3600(self, monkeypatch):
        ...
        assert weekly_insights.WEEKLY_INSIGHTS_CACHE_TTL_SECONDS == 3600
